...
# Allow running as `python backend/vector_ingestor.py` from the project root.
...
from backend.logger import (Colors, log_error, log_header, log_info,
...
if not os.getenv("TAVILY_API_KEY"):
    log_error(
        "TAVILY_API_KEY is not set. Create a .env file in the project root "
...
async def main():
    ...
    for tavily_crawl_result_item in res["results"]:
        ...
        if not raw_content:
            log_warning(f"TavilyCrawl: Skipping {url} — no content extracted")
            continue
        log_info(
